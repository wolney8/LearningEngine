...
For the Phase 10 AI roadmap, Gemini is configured through backend environment variables (`GEMINI_API_KEY` and `GEMINI_MODEL`). Even when YAML is generated by an AI assistant, treat it as a draft and run it through the backend validation flow before publishing.
